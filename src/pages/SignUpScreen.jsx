...
function SignUpScreen() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const { role } = await signUpWithEmail(email, password, fullName)
      navigateByRole(role)
    } catch (err) {
      const code = err.code
    ...
    <div className="signup-container">  
      <div className="signup-card">
        ...
        <form className="signup-form" onSubmit={handleSubmit}>
          <div className={`input-group floating-label ${fullName ? 'has-value' : ''}`}>
            <input
              id="fullname-input"
              type="text" 
              value={fullName}
          ...
          <div className={`input-group floating-label ${password ? 'has-value' : ''}`}>
            <div className="password-wrapper">
              ...
              >
                {showPassword ? (
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="n  one" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className={`input-group floating-label ${confirmPassword ? 'has-value' : ''}`}>
            <input
              ...
              required
            />
            <label htmlFor="confirm-password-input" className="float-label">Confirm Password</label>
          </div>

          <button id="signup-submit-btn" type="submit" className="btn-signup" disabled={loading}>
        ...
        {/* Google button */}
        <button id="google-signup-btn" className="btn-google" onClick={handleGoogleSignUp} disabled={loading}>
